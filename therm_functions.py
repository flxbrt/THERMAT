...
#         s2 = s1 + h
#         ri = radial[l_index] + tl*np.cos(phi)
#         ro = radial[l_index] + (tl+h)*np.cos(phi)
#         # https://studyflix.de/mathematik/oberflaeche-kegel-2612
#         A = np.pi*(s2*ro - s1*ri)
#         A_cross_per_channel[l_index] = A/N_channels - h*tc
...
#         circum_outer = Ro*2*np.pi      
#         circum_inner = Ri*2*np.pi
#         circum_per_channel[l_index] = (circum_outer+circum_inner)/N_channels - 2*tc + 2*h
#     dia_hydraulic = 4*A_cross_per_channel/circum_per_channel
...
# def solve_regen_cooling_node(substance, m_dot_coolant, d_th, r_hg, r_inner_c, node_length, dia_hyd, h, tc, w, A_cross_hg, A_hotgas, A_cooling, A_cross_cooling, lam_wall, T_aw, p_cc, T_c, p_c, h_coolant, c_star, T0, mu_hg, gamma_hg, Cp_hg, Pr_hg, Ma, init, eta_f_flag):
    
#     hc, lam_coolant = heat_transfer_cooling(m_dot_coolant, A_cross_cooling, dia_hyd, h_coolant, p_c, substance)
#     hc, eta_f = fin_efficiency(hc, h, tc, w, lam_coolant, eta_f_flag)
...
#     opti.subject_to(hg - hg_wo_sigma*sigma == 0)
#     opti.subject_to(hg_wo_sigma - (C / (d_th**0.2)) * ((mu_hg**0.2 * Cp_hg)/(Pr_hg**0.6)) * ((p_cc / c_star)**0.8) * ((A_star / A_cross_hg)**0.9) == 0)
#     # TODO: check if it correct that T0 appears and p_chamber --> static or total quantities?
#     opti.subject_to(sigma - (1 / (((((1 / 2) * (T_w1 / T0) * (1 + (((gamma_hg - 1) / 2) * Ma**2))) + (1 / 2))**(0.8 - (omega / 5))) * ((1 + (((gamma_hg - 1) / 2) * Ma**2))**(omega / 5)))) == 0)
    
#     opti.subject_to(Q_dot - A_hotgas*hg*(T_aw-T_w1) == 0)
#     opti.subject_to(Q_dot - A_cooling*hc*(T_w2-T_c) == 0)   
    
#     # TODO: imporve r_outer & r_inner prediction
#     opti.subject_to(Q_dot - (T_w1-T_w2)/(ca.log(r_inner_c/r_hg)/(lam_wall*2*ca.pi*node_length)) == 0)
...
#     # 3) obtain pressure losses
#     # TODO: rework delta p correlation
#     delta_p, rho, vel = pressure_loss(m_dot_coolant, A_cross_cooling, dia_hyd, node_length, h_coolant, p_c, substance)
#     # print(delta_p/1e5)
#     # delta_p = 0
#     p_c_next = p_c - delta_p
...
#     Q_dot_hg = A_hotgas*hg*(T_aw-T_w1)
#     Q_dot_conduction = (T_w1-T_w2)/(ca.log(r_inner_c/r_hg)/(lam_wall*2*ca.pi*node_length))
#     Q_dot_c = A_cooling*hc*(T_w2-T_c)
    
#     return hg, hg_wo_sigma, sigma, hc, T_w1, T_w2, Q_dot, q_dot_hg, T_c_next, p_c_next, eta_f, h2, Q_dot_hg, Q_dot_conduction, Q_dot_c, rho, vel
